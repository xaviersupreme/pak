...
#define WRITE_OPTIONS (PAK_OPT_COMPRESS | PAK_OPT_LEVEL | PAK_OPT_PATHS | PAK_OPT_EXCLUDE | PAK_OPT_NO_PAKIGNORE)
...
static const struct pak_command_spec command_specs[] = {
    ...
    { "list", "list", PAK_CMD_LIST, 1, 1, PAK_OPT_LONG, "pak list [--long] <archive.pak>" },
    ...
    { "verify", "verify", PAK_CMD_VERIFY, 1, 1, 0, "pak verify <archive.pak>" },
    { "test", "verify", PAK_CMD_VERIFY, 1, 1, 0, "pak test <archive.pak>" }
};
...
static int path_looks_like_archive(const char *path)
{
    ...
    return strcmp(path + len - 4, ".pak") == 0;
}
...
static void print_allowed_options(const struct pak_command_spec *spec, const struct pak_options *opts)
{
    ...
    if ((spec->allowed_options & WRITE_OPTIONS) != 0) {
        print_write_options(opts);
    } else if ((spec->allowed_options & PAK_OPT_LONG) != 0 && opts->long_list) {
        fputs(" --long", stderr);
...
void hint_no_command(int argc, char **argv, const struct pak_options *opts)
{
    ...
    if (opts != NULL && (opts->option_mask & WRITE_OPTIONS) != 0) {
        ...
        print_extract_options(opts);
        fputc('\n', stderr);
    } else if (opts != NULL && (opts->option_mask & PAK_OPT_LONG) != 0) {
        print_try_start();
        fputs(" list --long ", stderr);
        print_placeholder("<archive.pak>");
        fputc('\n', stderr);
    } else {
        print_try_start();
...
void hint_unknown_command(int argc, char **argv, int count, char **args, const struct pak_options *opts)
{
    ...
    diag_known("make, update, list, extract, unpack, cat, info, verify, test");
    if (suggestion == NULL) {
        return;
    }
